...
-- 21/06/2024
CREATE TABLE `otp_codes` (`id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT , `patient_id` BIGINT UNSIGNED NOT NULL , `created_by` BIGINT UNSIGNED NOT NULL , `otp_code` VARCHAR(255) NOT NULL , `is_active` TINYINT(1) NOT NULL , `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP , `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP , PRIMARY KEY (`id`)) ENGINE = InnoDB;
ALTER TABLE `otp_codes` ADD FOREIGN KEY (`created_by`) REFERENCES `users`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE; ALTER TABLE `otp_codes` ADD FOREIGN KEY (`patient_id`) REFERENCES `pattients`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
ALTER TABLE `users` ADD FOREIGN KEY (`role_id`) REFERENCES `roles`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
ALTER TABLE `users` ADD FOREIGN KEY (`member_id`) REFERENCES `members`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;